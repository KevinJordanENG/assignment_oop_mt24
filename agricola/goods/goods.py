...
class Supply:
    ...
    def __new__(cls, game: Game, *, num_food: int) -> Self:
        ...
        if not Player._is_constructing_supply():
            raise TypeError(
                "Supply can only be instantiated by 'Player', not directly."
            )
    ...
    def _count_person(
        self,
    ) -> tuple[int, tuple[Location, ...], tuple[Coordinate, ...]]:
        """Counts people and returns their locations."""
        count = 0
        temp_locs = []
        temp_coords = []
        for good in self.__limited_goods:
            if good["goods_type"] == "person":
                count += 1
                temp_locs.append(good["location"])
                temp_coords.append(good["coordinate"])
        return (count, tuple(temp_locs), tuple(temp_coords))
    ...
    def _pay(self, payment: tuple[tuple[int, str], ...]) -> None:
        ...
        for item in payment:  # For each unique good type / number tuple.
            for _ in range(item[0]):  # For num_goods of this goods type.
                ...
                fetch = self.__get_general_good(
                    goods_type, "inventory", (-1, -1)
                )
                if fetch is None:
    ...
    def __move_limited(
        ...
        if goods_type == "stable":
            ...
            if (
                source_board in {"farmyard", "action_space"}
                and destination_board == "inventory"
            ):
                raise ValueError("Illegal move requested.")
    ...
    def __move_general(
        ...
        for _ in range(num_goods):
            ...
            # inv -> farm.
                ...
                source_board == "inventory" and destination_board == "farmyard"
            ):
                fetch = self.__get_general_good(
                    goods_type, source_board, source_coord
                )
                if fetch is None:
            ...
            # act -> farm.
                ...
                and destination_board == "farmyard"
            ):
