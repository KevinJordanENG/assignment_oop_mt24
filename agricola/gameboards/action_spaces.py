...
NoEvalTokens: set[str] = (
    set(get_args(GoodsType)) | set(get_args(SpaceType)) | FuncNoEval
)
"""
...
class ActionSpaces(BaseBoard):
    ...
    def __init__(self, game: Game, num_players: int, path: str) -> None:
        ...
        if not Game._is_constructing_action_spaces():
            raise TypeError(
                "ActionSpaces board can only be instantiated by 'Game', not directly."
            )
        # Init BaseBoard.
        super().__init__()
        self._game = game
    ...
    def get_action_func_output(
        self, action: Action
    ) -> Any:  # Any used here as output is varied.
    ...
    def get_action_func_cost(
        self, action: Action
    ) -> Any:  # Any used here as cost is varied.
    ...
    def get_space_data_from_action(
        self, action: Action
    ) -> tuple[Coordinate, SpaceData] | None:
    ...
    def _add_action_space(self, round_num: int, stage: int) -> None:
        ...
        line: ActionCSVLine = self.__fetch_csv_line(
            round_num=round_num, stage=stage
        )
        # Add space to game board with bundled data.
        self._board[ROUND_COORDS[round_num - 1]] = self.__bundle_space_data(
            line
        )
    ...
    def __init_board_spaces(self, num_players: int) -> None:
        ...
        if num_players in (1, 2):
            ...
            self._valid_spaces = (
                START_COORDS["1_and_2_player"] | START_COORDS["3_player_adtl"]
            )
        elif num_players == 4:
            self._valid_spaces = (
                START_COORDS["1_and_2_player"] | START_COORDS["4_player_adtl"]
            )
    ...
    def __find_in_csv_data(self, key: str, value: Any) -> ActionCSVLine:
        ...
        if ans is None:
            raise ValueError(
                "Key / value pair for action data in CSV not found."
            )
        return ans

    def __select_rand_round_action(
        self, round_num: int, stage: int
    ) -> ActionCSVLine:
    ...
    def __fetch_csv_line(
        self, *, round_num: int, stage: int = 0, space: Coordinate | None = None
    ) -> ActionCSVLine:
        """
    ...
    def __bundle_space_data(
        self, csv_line: ActionCSVLine, num_players: int = -1
    ) -> SpaceData:
    ...
    def __load_csv(self, path: str) -> None:
        ...
        with open(
            os.path.join(path, "actions.csv"), "r", encoding="utf-8"
        ) as data:
