...
class Farmyard(BaseBoard):
    ...
    def __init__(self, game: Game) -> None:
        ...
        if not Player._is_constructing_farmyard():
            raise TypeError(
                "Farmyard can only be instantiated by 'Player', not directly."
            )
        super().__init__()
        self._game = game
        # Set board type.
    ...
    @property
    def board_perimeters(
        self,
    ) -> Mapping[tuple[Axis, Coordinate], MappingProxyType[str, object]]:
    ...
    def check_space_change_validity(
        self, space_type: SpaceType, coord: Coordinate
    ) -> bool:
        ...
        if space_type in {"wood_room", "clay_room", "stone_room"}:
            ...
            valid_current = (
                self._board[coord]["space_type"] == "unused"
    ...
    def _get_open_room(self) -> Coordinate:
        """Returns coord of space that is both open and of type room."""
        for space in self.open_spaces:
            if self._board[space]["space_type"] in {
                "wood_room",
                "clay_room",
                "stone_room",
            }:
                return space
        raise ValueError("Did not find open space to return 'person' home to.")

    def _build_fence(self) -> None:
        """Performs build fence action. Can only move fence from inventory to farmyard."""
        raise NotImplementedError()

    def _change_space_type(
        self, space_type: SpaceType, coord: Coordinate
    ) -> None:
    ...
    def __check_adjacency(
        ...
        up = (
            False
            if row - 1 < 0
            else (self._board[(row - 1, col)]["space_type"] == space_type)
        )
        down = (
            False
            if row + 1 > 2
        ...
        left = (
            False
            if col - 1 < 0
            else (self._board[(row, col - 1)]["space_type"] == space_type)
        )
        right = (
            False
            if col + 1 > 4
